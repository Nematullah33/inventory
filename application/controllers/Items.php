...
class Items extends MY_Controller {
	...
	public function get_json_items_details(){
		...
		//if (!empty($_GET['name'])) {
			...
			// echo "SELECT id,item_name,item_code,stock FROM db_items where  status=1 and  (LOWER(item_name) LIKE '%$name%' or LOWER(item_code) LIKE '%$name%' or LOWER(custom_barcode) LIKE '%$name%')   limit 10";
	...
	public function get_json_items_details_ext(){
		$data = array();
		$display_json = array();
		//if (!empty($_GET['name'])) {
			$id = strtolower(trim($_GET['id']));
			$sql =$this->db->query("SELECT id,item_name,item_code,stock FROM db_items where  status=1 and  id='$id'");
			// echo "SELECT id,item_name,item_code,stock FROM db_items where  status=1 and  (LOWER(item_name) LIKE '%$name%' or LOWER(item_code) LIKE '%$name%' or LOWER(custom_barcode) LIKE '%$name%')   limit 10";
			foreach ($sql->result() as $res) {
			      $json_arr["id"] = $res->id;
				  $json_arr["value"] = $res->item_name;
